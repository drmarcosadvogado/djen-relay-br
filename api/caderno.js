...
// e retorna apenas as comunicacoes cujos destinatarioadvogados batem com as
// OABs informadas. Roda no runtime Node (nao Edge) e na regiao gru1, definida
// em vercel.json, para poder alcancar comunicaapi.pje.jus.br sem geo-bloqueio.
//
// Query params:
...
//                            "182051-RJ,59214-RJ". Se omitido, retorna tudo.
...
module.exports = async (req, res) => {
    ...
    try {
      ...
      let filtradas = allComunicacoes;
          if (oabList) {
                  filtradas = allComunicacoes.filter(function (c) {
                            const advs = c.destinatarioadvogados || [];
                            return advs.some(function (d) {
                                        const adv = d.advogado || {};
                                        return oabList.indexOf(normOab(adv.numero_oab, adv.uf_oab)) !== -1;
                            });
                  });
          }
